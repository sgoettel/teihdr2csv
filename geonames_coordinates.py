...
def get_coordinates(place_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            lat_match = re.search(r'<wgs84_pos:lat>([^<]+)</wgs84_pos:lat>', xml_content)
            long_match = re.search(r'<wgs84_pos:long>([^<]+)</wgs84_pos:long>', xml_content)
...
def main():
    ...
    with open('teihdr_output.csv', 'r') as file:
        ...
        for i,row in enumerate(reader):
            letter_id, file_name, sender_name, sender_id, sender_place, sender_place_id, sender_place_lat, sender_place_long, date_sent, receiver_name, receiver_id, receiver_place, receiver_place_id, receiver_place_lat, receiver_place_long, title, url = row
            ...
            if sender_place_id and sender_place_id.startswith("https://www.geonames.org"):
                ...
                if lat is not None and long is not None:
                    ...
                    print(f"Couldn't find coordinates for sender place ID {sender_place_id} in row {i+1}")
                    row[6] = ""
                    row[7] = ""
            ...
            if receiver_place_id and receiver_place_id.startswith("https://www.geonames.org"):
                ...
                if lat is not None and long is not None:
                    ...
                    print(f"Couldn't find coordinates for receiver place ID {receiver_place_id} in row {i+1}")
                    row[13] = ""
                    row[14] = ""
            elif receiver_place_id:
                print(f"Invalid receiver place ID {receiver_place_id} in row {i+1}")
                row[13] = ""
                row[14] = ""
            else:
                row[13] = ""
